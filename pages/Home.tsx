...
const Home: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in flex flex-col min-h-[80vh]">
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-indigo-600 to-violet-600 dark:from-indigo-800 dark:to-violet-900 rounded-3xl p-8 md:p-12 text-white shadow-xl shadow-indigo-200 dark:shadow-none relative overflow-hidden">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-end">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Explore Categories</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {Object.values(SkillCategory).map((cat) => (
            <Link to={`/search?cat=${cat}`} key={cat} className="group bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm hover:shadow-md border border-slate-100 dark:border-slate-700 transition-all text-center">
              <div className="w-12 h-12 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-full flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
      ...
      <div className="space-y-4 flex-1">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Featured Swaps</h2>
          <Link to="/search" className="text-indigo-600 dark:text-indigo-400 font-medium flex items-center gap-1 hover:underline">
        ...
        {loading ? (
            ...
            {featuredSkills.map((skill) => {
                ...
                return (
                <div key={skill.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-lg transition-all group">
                    ...
                    <div className="absolute top-3 right-3 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm px-2 py-1 rounded-lg text-xs font-bold text-slate-700 dark:text-slate-200 shadow-sm">
                        {skill.category}
                    </div>
                    </div>
                    <div className="p-4">
      ...
      <footer className="mt-12 border-t border-slate-200 dark:border-slate-800 pt-10 pb-6 text-slate-500 dark:text-slate-400">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
              <div className="col-span-1 md:col-span-1">
                   <div className="flex items-center gap-2 mb-4">
                        ...
                        <h2 className="text-xl font-bold text-slate-800 dark:text-white">Swaply</h2>
                   </div>
